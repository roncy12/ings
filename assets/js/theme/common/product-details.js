...
export default class ProductDetails {
    ...
    addProductToCart(event, form) {
        ...
        this.$overlay.show();
        var radios = [];
        //store all radios checked into array

        var pg =  $('[data-product-option-change] input');
        $.each(pg,function(i,v){
            if($(v).is(":checked")){
                radios[i] = $(v);
                console.log(i);
            }
        });
        //remove checked of the radios to send the request in form to only product will be added to cart no other picklist first

        $.each(pg,function(x,v){
            if($(v).attr('shitty_data')){
        ...
        utils.api.cart.itemAdd(this.filterEmptyFilesFromForm(new FormData(form)), (err, response) => {
            ...
            if (this.previewModal) {
                ...
                var self = this;  /* to gain access inside the ajax (this) */
                /* product id of radios */
                let  product_ids = [];

                $.each(radios,function(radios_key,radios_val){
                    $(radios_val).prop('checked',true);
                    /* store the radios product id to product id */
                    product_ids.push($(radios_val).attr('shitty_data'));
                   
                });
                //remove undefined in array collection
                product_ids = product_ids.filter(e => e !== undefined);
                if(product_ids.length > 1){
                    $('#form-action-addToCart').val("PLEASE WAIT...");
                    $.each(product_ids,function(key,valz){
                        //use timeout function to call the please wait before calling the ajax that async = false
                        setTimeout(function(){
                            $.ajax({
                                url: "/cart.php?action=add&qty=1&product_id=" + valz,
                                type: 'GET',
                                async: false,
                                cache: false,
                                timeout:30000,
                                ...
                                success:function(result){
                                    console.log("picklist of product id: " + valz + " was successfully added to cart");
                                    if(key == product_ids.length - 1){
                                        //unchecked the picklist after the last iterate
                                        //please if its in admin and going to add another pick list please update this code also
                                            self.previewModal.open();
                                            self.updateCartContent(self.previewModal, response.data.cart_item.id);
                                    }
